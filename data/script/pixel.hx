...
function onNoteCreation(event) {
	...
	if (event.note.isSustainNote) {
		note.loadGraphic(Paths.image('stages/Vs Neon/ui/arrowEnds'), true, 7, 6);
		note.animation.add("hold", [event.strumID]);
		note.animation.add("holdend", [4 + event.strumID]);
	} else {
		note.loadGraphic(Paths.image('stages/Vs Neon/ui/arrows-pixels'), true, 17, 17);
...
function onStrumCreation(event) {
	...
	strum.loadGraphic(Paths.image('stages/Vs Neon/ui/arrows-pixels'), true, 17, 17);
...
function onCountdown(event) {
	...
	if (event.soundPath != null) event.soundPath = 'Vs Neon/' + event.soundPath;
	event.antialiasing = false;
	event.scale = daPixelZoom;
	event.spritePath = switch(event.swagCounter) {
		case 0: null;
		case 1: 'stages/Vs Neon/ui/ready';
		case 2: 'stages/Vs Neon/ui/set';
		case 3: 'stages/Vs Neon/ui/go';
	};
}

function onPlayerHit(event:NoteHitEvent) {
	if (!enablePixelUI) return;
	event.ratingPrefix = "stages/Vs Neon/ui/";
