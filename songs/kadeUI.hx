// Kade UI + some behavioral changes from Kade (not finished at all, but I'll get back into it when it's pirority)
...
function onPostStrumCreation(e) {
    ...
    if (kadeUI) e.strum.animation.addByIndices("pressed", e.animPrefix + " press", [0, 1, 2, 3], "", 12, false); // 12 frames when pressing (pretty minor detail :shrug:)
